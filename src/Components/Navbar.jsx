...
const Navbar = () => {
  ...
  return (
    <nav className=' fixed top-0 flex justify-between bg-opacity-5 md:justify-center  text-[#03045e] bg-[#d5f1f7] w-[100%]  my-4 mx-3 z-10  '>
      ...
      <img className='w-[6rem] h-[6rem] mx-2' src={smarttalk} alt="logo" />
      </Link>
   
        {
            !isOpen && <button className=' absolute top-0 right-6 h-6 md:hidden  z-10 ' onClick={()=>{
                setIsOpen(true)  
              }}>
                ...
                className={`  flex justify-between absolute  bg-[#d5f1f7] md:static ${isOpen === true ? " right-0 ":" -right-[100%] "} transition-all ease-in-out `}>
     
        {isOpen &&  <button className='absolute top-0 right-6 md:hidden' onClick={()=>{
              setIsOpen(false)  
            ...
            <ul className='flex flex-col  md:flex-row  relative my-6 gap-6 mx-10 font-bold' >
                ...
                <p className='text-[0.7rem] text-center'>About</p>
